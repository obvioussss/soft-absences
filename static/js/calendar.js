...
class Calendar {
    ...
    async showMonthlyView() {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/calendar/admin?year=${this.currentYear}&month=${this.currentMonth + 1}`, {
                headers: {
    ...
    async showYearlyView() {
        ...
        try {
            const [eventsResponse, summaryResponse] = await Promise.all([
                fetch(`${API_BASE_URL}/calendar/user?year=${this.currentYear}`, {
                    headers: {
                        'Authorization': `Bearer ${authToken}`
                    }
                }),
                fetch(`${API_BASE_URL}/calendar/summary?year=${this.currentYear}`, {
                    headers: {
                        'Authorization': `Bearer ${authToken}`
                    }
                })
